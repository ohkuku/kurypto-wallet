...
export const resolvers: Resolvers<{
  ...
  Query: {
    ...
    getTokenDetails: async (_, { tokenAddress }, { cache }) => {
      const queryTokenDetailsResult = cache.readQuery<
        GetTokenDetailsQuery,
        GetTokenDetailsQueryVariables
      >({
      ...
      if (!queryCurrentNetworkResult?.getCurrentNetwork)
        throw new Error('Please specify blockchain network');

      const provider = getProvider(
        queryCurrentNetworkResult.getCurrentNetwork as unknown as SupportedNetworks
      );
